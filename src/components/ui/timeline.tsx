...
const TimelineIcon = ({
  ...
  // 根据 status 自动选择颜色
  const statusColorMap: Record<string, 'primary' | 'secondary' | 'muted' | 'accent' | 'destructive'> = {
    completed: 'primary',
    'in-progress': 'accent',
    pending: 'muted',
    error: 'destructive',
  ...
  const effectiveColor = color || statusColorMap[status] || 'primary';
...
const TimelineEmpty = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, children, ...props }, ref) => (
    <div
      ...
      {...props}
    >
      <p className="text-sm text-muted-foreground">{children || '暂无数据'}</p>
    </div>
  ),
);
